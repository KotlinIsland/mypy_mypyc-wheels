...
const EXPECTED_ERROR = `test.py:1:5: error: Unsupported operand types for + ("int" and "str")  [operator]
  ...
  try {
    pyodide.runPython(`
      with open('test.py', 'w') as f:
          f.write("1 + ''")
    `);
    return pyodide.runPython("import mypy.api; mypy.api.run(['test.py', '--no-color-output', '--no-pretty', '--hide-error-code-links'])");
  } catch (e) {
